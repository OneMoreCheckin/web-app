...
//			url = 'https://api.foursquare.com/v2/venues/search?oauth_token=' + m_oauth + '&v=20110918&ll=' + (lat + "," + lng) + '&=' + cat;
		else
...
/*		else if(venues){
			url = 'https://api.foursquare.com/v2/multi?oauth_token=' + m_oauth + '&requests=';
			var req = [];
			for(var x = 0; x < venues.length; x++){
				req.push("/venues/search?ll" + encodeURIComponent());
			}
		}*/
		...
		$.ajax({
				...
				success: function (data) {
					if (data.error) 
						...
						m_ready = true;
						success();
						_gaq.push(['_setCustomVar', 1, 'uid', m_uid, 1], ['_trackPageview']);
//						Omc.Services.fetchInfo(function (d) { m_userdata = d; success();}, error);
					}
				},
				error : function (err) {
					error();
...
/*		$.ajax({
			url: 'https://api.foursquare.com/v2/users/self?oauth_token=' + m_oauth + '&v=20110918',
			dataType: "jsonp",
			data: {},
			success: function(data){
				success(data.response);
